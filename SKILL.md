...
→ Run its Context Graph → Scope Sizing → TODO Matrix → Permission Setup → Refactor → Verify workflow (10 dimensions: style, memory, performance/ANR, security, UI integrity, architecture, dead code, bugs, test coverage, accessibility) — reads the whole project first to build a module/dependency graph, asks once to auto-accept edits before the refactor loop instead of per-file, and attaches the graph in the final report alongside the TODO matrix
